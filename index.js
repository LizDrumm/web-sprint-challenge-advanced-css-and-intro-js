...
function getArtistByIndex(arr, id) {
  for (let i=0; i < arr.length; i++){
    if (arr[i].id === id){
        return `The artist at ${arr[i].id} is ${arr[i].name}`
      }
    }
  }
...
/* function get20s(/* Code here */
  ...
  function get20s (arr, min, max){
    let newArr= []
    for (let i=0; i <arr.length; i++){
    if (arr[i].years>=min && arr[i].years <=max){
...
bio: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus ac diam ornare enim volutpat tincidunt. Nam velit elit, rhoncus vel nisl ut, convallis aliquet urna. Suspendisse fermentum pretium massa, vel tristique dolor feugiat sit amet.' 
}
